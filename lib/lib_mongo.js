...
class oAPI{
    ...
    async open_mongodb(callback=undefined,params=undefined){
      ...
      //npm install mongodb@2.2.33 --save
        ...
        const url = 'mongodb://'+configuration.mongoConnection
    ...
    async group_by_day(){
        ...
        var cursor = await this.db.collection('caso').aggregate(
          {$group: {_id: {created_at:'$created_at',timestamp:'$timestamp'}, casos: {$sum: 1}}},
    ...
    async get_warms(){
        let warms = [];
        var cursor = await this.db.collection('warning').find({}).sort({'datatime':1});
        for (let doc = await cursor.next(); doc != null; doc = await cursor.next()) {
            warms.push(doc);
    ...
    async scores(data=[]){
        ...
        await this.db.collection('score').remove();
        for (let i in data) {
    ...
    async getScores(filter){
        let scores = [];
        var cursor = await this.db.collection('score').find({}).sort(filter);
        for (let doc = await cursor.next(); doc != null; doc = await cursor.next()) {
            scores.push(doc);
        }
        this.SOCKET.emit("getScores", scores);
    }
    async getCasos(){
        let casos = [];

...
class oAPI{
    ...
    async scores(doenca,data=[],socket){
        ...
        socket.emit("save", doenca, "process scores ok - "+doenca);
        return true;
    }
    async getScores(disease='',filter={}){
        let scores = [];
        var cursor = await this.db.collection('score').find({'doenca':disease}).sort(filter);
        for (let doc = await cursor.next(); doc != null; doc = await cursor.next()) {
            //if(doc.score.item != undefined)
            scores.push(doc);
        }
        this.SOCKET.emit("getScores", disease, scores);
    }
    async getCasos(){
        let casos = [];

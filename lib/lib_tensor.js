...
class oAPI{
   ...
   /*
      Função que configura a rede.
   */ 
    async execTensor(test){
        if(test.length >= 7){
            const target = tf.tensor2d([test]); 
            let ru = await this.model.predict(target); 
    ...
    async execPredictTensor(doenca,data){
        ...
        if(scores[0] != undefined){
            ...
            this.modelPredict.fit(xs, ys, {
                 ...
                 let ru = this.modelPredict.predict(tf.tensor2d([(scores.length)], [1, 1]));
    ...
    */    
    ...
    /*
       Monta os dados para treinamento.
    */ 
    mount_test(list){
